...
En aquest treball ens dedicarem a estudiar com afecta dur una vida activa i saludable als estudis, sota la hipòtesi de que dur una vida activa i saludable implica millor rendiment acadèmic. Per això mirarem les **hores d'activitat física** que fa un alumne a la setmana, les **hores de son** de mitjana al dia, **mitjana expedient,** i el **nombre de menjades "trampa"** que fa un alume per setmana. Per altre banda, es demanarà el **Nivell d'energia percebut**: Classificat com "baix", "mitjà" o "alt", la **percepció del nivell d'estrès**: Classificat com "baix", "moderadament alt" o "alt" i la **satisfacció amb el propi estil de vida**: En una escala del 1 al 5 (sent 1 = gens satisfet i 5 = molt satisfet). A part d'això, també ens interessa la **modalitat del grau** estudiat: (ciències, filologia, activitat física,...) i el **Tipus d'activitat física** que l'alumne fa: (córrer, nedar, ciclisme, ioga, etc.). Anem a simular una mostra de 100 alumnes a partir d'una distribució normal multivariant i la funció sample()
...
Amb aquest diagrama de violí, podem observar que no hi ha gaire diferència en com es distribueixen, podríem fer un altre test d'ANOVA i veure si les mitjanes són iguals.

```{r}
summary(aov(dades$Mitjana_Expedient~dades$Tipus_Activitat))
```

Efectivament, tenim un $p$-valor prou alt (0.769) i acceptem la hipòtesi nul·la de que les mitjanes són totes igual.
...
Ara realitzarem el mateix contrast, però de manera teòrica. Calculant l'estadístic del test de Hotelling $T^2$ , i relacionant-lo amb la distribució de Fisher.

```{r}
S=dades_filtrades %>% dplyr::select(1:4) %>% as.matrix() %>% cov()
